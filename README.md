...
## 规则,节点策略：
1. **规则并非越细越好**：
* **性能考量**：每一条规则都会消耗 CPU 进行正则或匹配计算。过度细化的规则会增加内存开销并拖慢连接建立的速度（TTFB）。
* **可维护性**：规则集越臃肿，排查“为何流量走错”的难度就越大。应优先使用 `rule-set` 的二进制 (`.srs`) 格式，将海量域名合并，仅在必要时才手动编写单条规则。


2. **节点选择的策略**：
* **节点瘦身**：节点列表过长会增加 `sing-box` 初始化及选择过程的负担，同时会拖慢客户端侧选择延迟检测的频率。
* **优选机制**：应只保留常用、稳定、低延迟的节点。对于不活跃或备用的节点，建议单独放置在备用配置文件中，避免在核心配置中造成干扰。
